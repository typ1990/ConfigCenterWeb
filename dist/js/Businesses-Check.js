...
function submitClick(){
    ...
    var ob=new Object();
    ob.id=shopId;
    ob.name=$("#shopName").val();
    ob.superShopShortName=$("#shopShortName").val();
    ob.branchCompanyName=branName;
    ...
    shopId="";//添加完id清空
    var obw=new Object();
    obw.marked="superShop_addAndEdit";
...
function addperson(){
    ...
    if(CheckIsMobile(phone11)){
        ...
        var strHtml = "<tr id=tr_id><th>"+$('#name1').val()+ "</th>" +
            ...
            "<th><a href='#' onclick='xiugai(this)'>修改</a><a  href='#' onclick='deleterow(this)'>删除</a></th></tr>";
...
function callBack_selectProvice(data){
    ...
    //    branchId="";
    //}else{
    $("#branchText").val($.cookie("branchName"));
    branchId=$.cookie("branchId");
        //如果是编辑状态查询单条记录
        if(editFlag){
            seleOne();
        }
